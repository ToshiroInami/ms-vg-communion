...
import { CommunionimgModalService } from '../communionimg-modal/communionimg.service';
...
export class AdminCommunionListComponent {
  ...
  imageSrc: string = '/assets/img/demobautismo.jpg';

  constructor(private communionService: CommunionService, private communionimgModalService: CommunionimgModalService) { }
  ...
  editCommunion(communion: Communion): void {
    this.communionToEdit = { ...communion, communionDate: this.formatDate(communion.communionDate) };
  }
  ...
  formatDateTable(date: string | null): string | null {
    if (!date) return null;
  ...
  saveCommunion(): void {
    if (this.communionToEdit) {
      ...
      const changesMade =
        ...
        this.communionToEdit.communionDate !== originalCommunion.communionDate ||
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          this.communionService.updateCommunion(this.communionToEdit!).subscribe(
            (updatedCommunion: Communion) => {
              console.log('Comunion actualizada correctamente', updatedCommunion);
              Swal.fire(
                'Actualizado',
            ...
            (error) => {
              ...
              Swal.fire('Error', 'No se pudo actualizar la comunion', 'error');
            }
          );
        } else {
  ...
  filterCommunions(): void {
    if (this.selectedCommunion) {
      ...
      this.filteredCommunions = this.communions;
    }
    this.totalItems = this.filteredCommunions.length;
    this.getPaginatedCommunions();
  }

  /* Paginador */

  getPaginatedCommunions(): void {
    const startIndex = (this.currentPage - 1) * this.itemsPerPage;
    const endIndex = startIndex + this.itemsPerPage;
    this.displayedCommunions = this.filteredCommunions.slice(startIndex, endIndex);
  }
  ...
  onDownloadExcel(): void {
    const exportData = this.filteredCommunions.map((communion) => ({
      Nombres: communion.names,
      Apellidos: communion.surnames,
      LugarComunion: communion.placeCommunion,
      // Más campos
    }));

    const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(exportData);
    const wb: XLSX.WorkBook = { Sheets: { data: ws }, SheetNames: ['data'] };
    XLSX.writeFile(wb, 'comuniones.xlsx');
  }

  onDownloadCSV(): void {
    const exportData = this.filteredCommunions.map((communion) => ({
      Nombre: communion.names,
      Apellido: communion.surnames,
      LugarComunion: communion.placeCommunion,
      // Más campos
    }));
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute('download', 'comuniones.csv');
  ...
  openImageModal(): void {
    this.communionimgModalService.open(this.imageSrc);
  }
  ...
  onSearchChange(event: any): void {
    ...
    this.displayedCommunions = this.communions.filter(communion =>
        (communion.names || '').toLowerCase().includes(searchTerm) ||
        (communion.surnames || '').toLowerCase().includes(searchTerm) ||
        (communion.placeCommunion || '').toLowerCase().includes(searchTerm) ||
        (communion.priest || '').toLowerCase().includes(searchTerm)
    );
}
